...
public class L2149_RearrangeArrayElementsBySign {
    ...
    public void recursion(int[] arr, Stack<Integer> stackP, Stack<Integer> stackN, int index){
        if(index == arr.length){
            return;
        }

        int a = (arr[index] > 0)?stackP.push(arr[index]):stackN.push(arr[index]);
        a = (arr[index+1] > 0)?stackP.push(arr[index+1]):stackN.push(arr[index+1]);

        recursion(arr, stackP, stackN, index+2);
