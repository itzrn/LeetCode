...
public class L2870_MinimumNumberOfOperationsToMakeArrayEmpty {
    /*
    * count = 3
    3 - 3 = 0
...
//        var counter = new HashMap<Integer, Integer>(); // like this also we can initialize any data structure
        for(int i :nums){ // keeping the count of every variable
            map.put(i, map.getOrDefault(i, 0)+1);
        }


        for(int i:map.values()){
            if(i == 1){ // if count is 1, then only it's possible that there will be no valid case
                return -1;
            }

            // else there always be a valid number of operations  to be done
            operations += Math.ceil((double) i/3);
        }
