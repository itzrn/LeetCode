...
public class L841_KeysAndRooms {
    public boolean canVisitAllRooms(List<List<Integer>> rooms) {
        boolean[] visited = new boolean[rooms.size()]; // its working like dp here
        recursion(rooms, 0, visited);
        for(boolean i: visited){ // after dfs checking if all the room are visited
            if(!i){ // if any room not visited returning false
                return false;
            }
        }
        return true;
    ...
    // doing normal dfs
    public void recursion(List<List<Integer>> rooms, int index, boolean[] visited){
        if(visited[index]){
            return;
        }
