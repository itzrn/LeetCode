...
class L981_Pair {
    ...
    L981_Pair(String value, int timestamp){
        this.value = value;
        this.timestamp = timestamp;
    }
}
public class L981_TimeBasedKeyValueStore {
    Map<String, List<L981_Pair>> map;
    ...
    public void set(String key, String value, int timestamp) {
        ...
        map.get(key).add(new L981_Pair(value, timestamp));
    ...
    public String get(String key, int timestamp) {
        ...
        List<L981_Pair> list = map.get(key);
        int low = 0;
        int high = list.size()-1;
        binarySearch(list, low, high, timestamp);
        return ans;
    }

    String ans = "";
    public void binarySearch(List<L981_Pair> list, int low, int high, int timestamp){
        if(low>high){
            return;
        }
        ...
        if(time == timestamp){ // if the particular time stamp is already available
            ans =  list.get(mid).value;
        }else if(time < timestamp){
            ans = list.get(low).value; // this is returning the value associated with the largest timestamp_prev
            binarySearch(list, mid+1, high, timestamp);
        }else{
            binarySearch(list, low, mid-1, timestamp);
        }
    }
}
