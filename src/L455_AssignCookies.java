...
public class L455_AssignCookies {
    public int findContentChildren(int[] g, int[] s) {
        ...
        while(i<n && j<m-1){ // condition till which we have to go for
            while(j<m-1 && g[i] > s[++j]){
                continue;
            }

            if(g[i] <= s[j]){ // this condition is bcz, it can happen that the upper loop terminate at the condition j<m-1
                // at that time its possible g[i] > s[j], at which we don't want to increase the count
                count++;
            }
            i++;
        }
