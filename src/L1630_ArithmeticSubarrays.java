...
public class L1630_ArithmeticSubarrays {

    // approach 1 -> brut force
    public List<Boolean> checkArithmeticSubarrays(int[] nums, int[] l, int[] r) {
        List<Boolean> ans = new ArrayList<>();
        for(int i=0; i<l.length; i++){
            int[] arr = new int[r[i]-l[i]+1]; // creating an array

            for(int j = l[i], index = 0; j<=r[i]; j++, index++){
                arr[index] = nums[j]; // filling the arr, with element which is need to check those are in sequence or not
            }

            Arrays.sort(arr);// sorting the array
            ans.add(check(arr)); // checking for sequence
        }
    ...
    // checking the given sequence is AP or not
